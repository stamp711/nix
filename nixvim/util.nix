{
  flake.nixvimModules.default =
    # Shared lua helpers. Root() / Root.git(): project root — full mirror of LazyVim's util/root.lua.
    {
      extraConfigLua = ''
        do
          -- Full port of LazyVim (lua/lazyvim/util/root.lua), exposed as the global Root.
          ...
          function M.info()
            ...
            lines[#lines + 1] = "```"
            vim.notify(table.concat(lines, "\n"), vim.log.levels.INFO, { title = "Roots", ft = "markdown" })
            return roots[1] and roots[1].paths[1] or vim.uv.cwd()
          end

          function M.setup()
            vim.api.nvim_create_user_command("Root", function() M.info() end, { desc = "Roots for the current buffer" })
