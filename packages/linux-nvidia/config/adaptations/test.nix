...
let
  ...
  adaptations = import ./.;
  overridesUnchanged = lib.all (
    arch:
    lib.all (
      name:
      let
        actual = configsByArch.${arch}.${"CONFIG_${name}"} or null;
        expected = adaptations.overrides.${name}.upstream;
      in
  ...
  computedStillDefined = lib.all (
    ...
    lib.all (
      ...
      true
    ) adaptations.computedByFile.${file}
  ) (builtins.attrNames adaptations.computedByFile);
in
assert overridesUnchanged;
assert computedStillDefined;
